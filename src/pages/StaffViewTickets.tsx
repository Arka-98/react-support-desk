...
import { getTicketsByStaffId, getTicketsByUserId, reset, resetExceptTickets } from '../features/ticketSlice'

function StaffViewTickets() {
    const location = useLocation()
    const [searchParams, setSearchParams] = useSearchParams()
    const { tickets, count, isError, isLoading, isSuccess, message } = useAppSelector(state => state.ticket)
    ...
    useEffect(() => {
        const status = (searchParams.get('resolved') === 'true') ? 'closed' : 'new,open'
        reduxDispatch(getTicketsByStaffId(status))
    }, [])
    ...
    return isLoading ? <Spinner/> : (
        <>
            ...
            <header className='flex flex-col gap-4 items-center'>
                ...
                <p>
                {
                    (searchParams.get('resolved') === 'true') ? `${tickets?.length} ${tickets?.length === 1 ? 'Ticket' : 'Tickets'} Resolved` : 
...
export default StaffViewTickets